...
def get_target_paths(source_paths: typing.List[pathlib.Path],
    ...
    default_target_dir = pathlib.Path('results/')
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help="Name on AttGAN model"
    )
    parser.add_argument(
        "--checkpoint", default=None, type=str,
        help="Choose checkpoint to load from"
    )
    parser.add_argument(
        "--skin", default=None, type=int,
        help="'-1': dark, '1': light"
    )
    return parser
...
def main():
    ...
    anonymizer, cfg = build_anonymizer(args.experiment_name, args.checkpoint, args.skin,
